...
3. **Configure Webhook**:
   ...
   - Set "WHEN A MESSAGE COMES IN" to: `https://legislator-hub.preview.emergentagent.com/api/whatsapp/webhook`
   - Method: POST
   - Click Save
...
**Send Message**:
```bash
curl -X POST "https://legislator-hub.preview.emergentagent.com/api/whatsapp/send" \
...
**Status Check**:
```bash
curl "https://legislator-hub.preview.emergentagent.com/api/whatsapp/status"
```

**Broadcast** (send to all constituents):
```bash
curl -X POST "https://legislator-hub.preview.emergentagent.com/api/whatsapp/broadcast?politician_id=<id>&message=Important announcement"
```
